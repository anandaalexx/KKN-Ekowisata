  ...
  return (
    <div className="w-[405px] flex flex-col">
      <img 
        src={image} 
        alt={name} 
        className="w-full h-76 object-cover rounded-t-md" 
      />
      {/* Overlay untuk nama dan harga */}
      <div className="px-3 py-2 bg-[#6CAC5C] text-white  text-xl shadow-lg -mt-4 mx-3 z-10 relative text-left rounded-md">
        {name} <br /> <span className="text-3xl text-shadow-lg font-bold">Rp. {price}</span>
      </div>
        ...
        <ul className="space-y-0.5">
          {(descriptionList || defaultDescList).map((item, index) => (
            <li key={index} className="flex items-start">
              <Play 
                size={16} 
                className="w-4 h-4 mr-2 mt-[3px] text-[#8DC26F] flex-shrink-0"
                fill="#6CAC5C"
              />
              <span>{item}</span>
            </li>
...
TicketCard.defaultProps = {
  descriptionParagraph: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
  descriptionList: [
