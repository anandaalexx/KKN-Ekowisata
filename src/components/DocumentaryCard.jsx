...
export default function DokumenterCard({
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row w-full overflow-hidden p-4 sm:p-6">
      {" "}
      {/* Padding ditambahkan di sini untuk konten internal */}
      {/* Bagian Gambar */}
      ...
      <div className="w-full md:w-[35%] lg:w-[30%] relative h-48 md:h-auto aspect-video md:aspect-auto rounded-lg overflow-hidden bg-gray-200">
        {" "}
        {/* rounded-lg untuk gambar, bg-gray-200 sbg placeholder */}
        <Image
          src={image}
        ...
        <div>
          ...
          <div className="space-y-1 text-sm text-gray-700 mb-4">
            {listItems && listItems.length > 0 ? (
              listItems.slice(0, 3).map(
                (
                  item,
                  index // Tampilkan maks 3 item
                ) => (
                  <div key={index} className="flex items-start">
                    <Play
                      size={15}
                      className="w-3.5 h-3.5 mr-2 mt-[3px] text-green-600 flex-shrink-0"
                      fill="currentColor"
                    />
          ...
          >
            ...
            <div className="flex items-center ml-1.5 sm:ml-2">
              ...
              <ChevronRight
                size={16}
                className="-ml-2 sm:-ml-2.5 transition-transform duration-200 ease-in-out group-hover:translate-x-0.5"
              />
            </div>
          </button>
        </div>
...
DokumenterCard.defaultProps = {
  ...
  summary:
    "Ini adalah ringkasan default untuk dokumenter. Informasi lebih lanjut akan tersedia.",
  listItems: [
